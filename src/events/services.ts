...
export class EventsServices extends EventsHelpers {
  ...
  public async registerService(userData: Partial<IEventUser>): Promise<IEventUser> {
  ...
  public async activateEventService(event_code: string, op: string): Promise<IEvent> {
    logger('activateEventService1', LogTypes.LOGS);
    const event = await this.activateEventHelper(event_code, op);
    if (!event) {
      if (op === 'activate') {
        throw new ErrorHandler({
          status_code: 500,
